...
public partial class enemyC : RigidBody2D
{
	...
	public void processMovement()// every frame called
	{
		...
		if (speedAdjusted && speedAdjustUntill < Time.GetTicksMsec()) calculateSpeedAdjust();
	...
	void calculateSpeedAdjust()
	{
		...
		for (int i = 0; i < speedModifiers.Length; i++)
		{
			...
			if (speedModifiers[i].imEnabled == false) continue;
			else if (speedModifiers[i].until < Time.GetTicksMsec())
			{ speedModifiers[i].imEnabled = false; continue; }

			speedAdjusted = true;

			if (speedModifiers[i].adjust < 0) temorarySpeed -= speedModifiers[i].adjust;
			else temorarySpeed += speedModifiers[i].adjust - 1;

			if (speedAdjustUntill > speedModifiers[i].until) speedAdjustUntill = speedModifiers[i].until;



		}

		if (temorarySpeed < 0) temorarySpeed = 0f;
		else temorarySpeed *= speed;
