...
public partial class activeCardInfo : Node2D
{
    ...
    [Export] Label cardName, cardInfo;
    [Export] float wordDelay = 0.05f;
    [Export] Vector2 panelPositions;

    string[] infoWords = new string[0];
    int wordCounter; float timer;


    Card selectedCardHolder;
    int cardOffset = -10;
    float sizeHolder, positionHolder;
    bool setUpDone;
    ...
    void selectedCardGraphics()
    {
        ...
        selectedCardHolder.Position = new Vector2(selectedCardHolder.Position.X
        , positionHolder + cardOffset);
    ...
    {
        if (!IsInstanceValid(selectedCardHolder)) return;
    ...
    public override void _Process(double delta)
    {
        ...
        timer += (float)delta;
        if (timer < wordDelay) return;
        if (wordCounter >= infoWords.Length) return;
        timer = 0;
