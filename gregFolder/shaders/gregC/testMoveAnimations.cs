...
public partial class testMoveAnimations : Node2D
{
	...
	public override void _Process(double delta)
	{
		...
		if (Position.Y > moveLimits.Y) direction = -1;
		else if (Position.Y < moveLimits.X) direction = 1;
	...
	int side = 1;
	[Export]float moveDistance = 0.3f;
	[Export]float moveRotation = 1;
	[Export]float moveCooldown = 0.3f;
	double moveTimer;

	[Export] Sprite2D body;
	[Export] Sprite2D shord;
	[Export] float moveSpeed;
	[Export] Vector2 moveLimits;
	int direction = 1;
